"""Leg Press (Feet High) exercise tracker"""
from utils.base_exercise import BaseExercise


class LegPressFeetHighCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for leg press (feet high position)
        ...
        if angle > self.max_angle_threshold:
            ...
            if self.stage != "flexed":
                ...
                self.counter += 1
                feedback = "Perfect! Drive up"
            else:
                feedback = "Hamstrings engaged"
        else:
            feedback = "Feet high - press"
