"""Bulgarian Split Squat exercise tracker"""
from utils.base_exercise import BaseExercise


class BulgarianSplitSquatCoordinates(BaseExercise):
    def __init__(self, user_id=None):
        super().__init__(user_id)
        self.min_angle_threshold = 80
        self.max_angle_threshold = 165
        
    def process_coordinates(self, coordinates):
        """
        Process coordinates for Bulgarian split squat
        ...
        # Bulgarian split squat logic
        if angle > self.max_angle_threshold:
            if self.stage != "up":
                self.stage = "up"
                current_stage = "up"
            feedback = "Standing on front leg"
