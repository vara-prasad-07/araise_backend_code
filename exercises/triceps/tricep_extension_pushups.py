"""Tricep Extension Push-ups exercise tracker"""
from utils.base_exercise import BaseExercise


class TricepExtensionPushupsCoordinates(BaseExercise):
    def __init__(self, user_id=None):
        super().__init__(user_id)
        self.min_angle_threshold = 75
        self.max_angle_threshold = 165
        
    def process_coordinates(self, coordinates):
        """
        Process coordinates for tricep extension push-ups
        ...
        # Tricep extension pushups logic
        if angle > self.max_angle_threshold:
            ...
            if self.stage != "down":
                ...
                current_stage = "down"
                self.counter += 1
                feedback = "Great! Push back up"
            else:
                feedback = "Elbows tucked in"
        else:
            feedback = "Close grip, elbows in"
