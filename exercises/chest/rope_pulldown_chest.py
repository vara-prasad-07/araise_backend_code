"""Rope Pulldown (Chest) exercise tracker"""
from utils.base_exercise import BaseExercise


class RopePulldownChestCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for rope pulldown (chest variation)
        ...
        if angle > self.max_angle_threshold:
            ...
            if self.stage != "down":
                ...
                current_stage = "down"
                self.counter += 1
                feedback = "Great contraction! Return"
            else:
                feedback = "Squeeze at bottom"
        else:
            feedback = "Pull down and out"
