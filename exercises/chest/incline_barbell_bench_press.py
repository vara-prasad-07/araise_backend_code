"""Incline Barbell Bench Press exercise tracker"""
from utils.base_exercise import BaseExercise


class InclineBarbellBenchPressCoordinates(BaseExercise):
    def __init__(self, user_id=None):
        super().__init__(user_id)
        self.min_angle_threshold = 65
        self.max_angle_threshold = 165
        
    def process_coordinates(self, coordinates):
        """
        Process coordinates for incline barbell bench press
        ...
        # Incline bench press logic
        if angle > self.max_angle_threshold:
            if self.stage != "up":
                self.stage = "up"
                current_stage = "up"
            feedback = "Lockout complete"
            ...
            if self.stage != "down":
                ...
                current_stage = "down"
                self.counter += 1
                feedback = "Perfect rep! Push up"
            else:
                feedback = "Press the bar up"
        else:
            feedback = "Control the weight"
