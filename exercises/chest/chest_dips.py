"""Chest Dips (leaning forward) exercise tracker"""
from utils.base_exercise import BaseExercise


class ChestDipsCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for chest dips
        ...
        # Chest dips logic (lean forward for chest emphasis)
        if angle > self.max_angle_threshold:
            if self.stage != "up":
                self.stage = "up"
                current_stage = "up"
            feedback = "Top position - lean forward"
