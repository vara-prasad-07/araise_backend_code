"""Dumbbell Lateral Raises exercise tracker"""
from utils.base_exercise import BaseExercise


class DumbbellLateralRaisesCoordinates(BaseExercise):
    def __init__(self, user_id=None):
        super().__init__(user_id)
        self.min_angle_threshold = 15
        self.max_angle_threshold = 90
        
    def process_coordinates(self, coordinates):
        """
        Process coordinates for dumbbell lateral raises
        ...
        if angle < self.min_angle_threshold:
            ...
            if self.stage != "up":
                ...
                current_stage = "up"
                self.counter += 1
                feedback = "Perfect height! Lower"
            else:
                feedback = "Hold at top"
        else:
            feedback = "Lift to sides"
