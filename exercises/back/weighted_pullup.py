"""Weighted Pull-ups exercise tracker"""
from utils.base_exercise import BaseExercise


class WeightedPullupCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for weighted pull-ups
        ...
        # Weighted pullup logic
        if angle > self.max_angle_threshold:
            if self.stage != "down":
                self.stage = "down"
                current_stage = "down"
            feedback = "Dead hang position"
            ...
            if self.stage != "up":
                ...
                current_stage = "up"
                self.counter += 1
                feedback = "Strong rep! Lower down"
            else:
                feedback = "Chin above bar"
        else:
            feedback = "Pull with control"
