"""EZ Bar Preacher Curls exercise tracker"""
from utils.base_exercise import BaseExercise


class EZBarPreacherCurlsCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for EZ bar preacher curls
        ...
        if angle > self.max_angle_threshold:
            ...
            if self.stage != "up":
                ...
                current_stage = "up"
                self.counter += 1
                feedback = "Perfect curl! Lower"
            else:
                feedback = "Peak contraction"
        else:
            feedback = "Curl up smoothly"
