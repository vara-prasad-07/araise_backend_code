"""Incline Dumbbell Curls exercise tracker"""
from utils.base_exercise import BaseExercise


class InclineDumbbellCurlsCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for incline dumbbell curls
        ...
        if angle > self.max_angle_threshold:
            if self.stage != "down":
                self.stage = "down"
                current_stage = "down"
            feedback = "Full stretch position"
            ...
            if self.stage != "up":
                ...
                current_stage = "up"
                self.counter += 1
                feedback = "Great stretch! Lower"
            else:
                feedback = "Biceps fully contracted"
        else:
            feedback = "Curl with control"
