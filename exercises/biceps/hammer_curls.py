"""Hammer Curls exercise tracker"""
from utils.base_exercise import BaseExercise


class HammerCurlsCoordinates(BaseExercise):
    ...
    def process_coordinates(self, coordinates):
        """
        Process coordinates for hammer curls
        ...
        if angle > self.max_angle_threshold:
            if self.stage != "down":
                self.stage = "down"
                current_stage = "down"
            feedback = "Arms down, neutral grip"
            ...
            if self.stage != "up":
                ...
                current_stage = "up"
                self.counter += 1
                feedback = "Perfect! Lower down"
            else:
                feedback = "Forearms engaged"
        else:
            feedback = "Curl up, hammer style"
